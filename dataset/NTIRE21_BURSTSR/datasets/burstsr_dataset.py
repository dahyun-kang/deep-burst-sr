...
class SamsungRAWImage:
    ...
    def get_image_data(self, substract_black_level=False, white_balance=False, normalize=False):
        ...
        # print(im_raw.shape)  # 4 * 80 * 80
        # print(im_raw.max(), im_raw.min(), self.black_level, self.cam_wb, self.norm_factor)
...
class BurstSRDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        '''
        for im in frames:
            print(im.im_raw.max(), im.im_raw.min())
        '''
        ...
        burst = torch.stack(burst_image_data, dim=0) * 0.001
        ...
        # return burst, frame_gt, meta_info_burst, meta_info_gt, '{}/{}/samsung_{:02d}'.format(self.root, self.burst_list[index])
        return burst, frame_gt, meta_info_burst, meta_info_gt, '{}/{}'.format(self.root, self.burst_list[index])
